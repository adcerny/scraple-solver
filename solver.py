...
def run_solver():
    ...
    parser.add_argument('--positions-per-word', type=int, default=1, help='How many starting positions to try for each word (default: 1)')
    ...
    positions_per_word = args.positions_per_word
    max_moves = args.depth
    log_with_time(
        f"Evaluating full {beam_width} beam width search with {first_moves or beam_width} first moves and max depth {max_moves} using {positions_per_word} position(s) per word..."
    )
